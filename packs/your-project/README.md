...
every skill is declared once in `pipeline.your-project.yaml`, then loaded into
...
| **Tier 2 skill (`layer_map`)** | `pipeline.your-project.yaml` `skills.layer_map` | Surgeon Step 0a — task `Layer:` field or file path matches |
| **Tier 2 skill (`extra_triggers`)** | `pipeline.your-project.yaml` `skills.extra_triggers` | When a layered orthogonal condition is met (e.g., a11y) |
| **Per-agent skill** | `pipeline.your-project.yaml` `skills.orchestrator.*` / `skills.explorer.*` | Once per agent invocation, regardless of task |
...
| File | Purpose |
|------|---------|
| `your-project-project-scope.mdc` | Repo boundary, tech stack, allowed/forbidden build commands, ticket prefix |
| `your-project-naming-conventions.mdc` | Universal naming rules — files, identifiers, classes |
...
|------|-----------|-------------|
| `your-project-angular18-standards.md` | `Frontend/Angular18` | Task touches `frontend/src/**/*.{ts,html}` OR `task.Layer = "Angular18"` |
| `your-project-angular19-standards.md` | `Frontend/Angular19` | Task touches `frontend/src/**/*.{ts,html}` OR `task.Layer = "Angular19"` |
| `your-project-react-standards.md` | `Frontend/React` | Task touches `src/**/*.{tsx,jsx}` OR `task.Layer = "React"` |
| `your-project-vue3-standards.md` | `Frontend/Vue` | Task touches `src/**/*.vue` OR `task.Layer = "Vue3"` |
| `your-project-javascript-standards.md` | `JavaScript` | Task touches `**/*.js` OR `task.Layer = "JavaScript"` |
| `your-project-typescript-standards.md` | `TypeScript` | Task touches `**/*.ts` OR `task.Layer = "TypeScript"` (also layered into React/Vue) |
| `your-project-java-standards.md` | `Backend/Java` | Task touches `backend/src/main/java/**/*.java` OR `task.Layer = "Java"` |

**A task only ever loads 1–3 of these**, never all 7. That's the whole
point of the trigger model.

### `pipeline.your-project.yaml` — the trigger map
...
### `project-context.your-project.md` — standing project context
...
1. **Write the skill** at `packs/your-project/skills/your-project-python-standards.md`.
   Structure: frontmatter (`name`, `description`), then one Markdown
   document. Look at any of the 7 existing skills as a template.

2. **Wire it in `pipeline.your-project.yaml`** under `skills.layer_map`:

   ```yaml
   "Backend/Python":
     skills: [your-project-python-standards.md]
...
4. **Re-install the pack** so the runtime YAMLs get regenerated:
   ...
   ```bash
   npm run install-pipeline -- --pack your-project --merge-config
   ```
...
1. **Write the rule** at `packs/your-project/rules/your-project-<topic>.mdc` with
   the frontmatter:
...
writes some YAML blocks back into `pipeline.your-project.yaml` (or the runtime
splits). Specifically:
...
# Claude Code
npm run install-pipeline:claude -- --pack your-project --project-root /path/to/your/project
```

The installer:
- Copies `rules/` into the host's `.cursor/rules/` or `.claude/rules/`
- Copies `skills/` into the host's `.cursor/skills/` or `.claude/skills/`
- Splits `pipeline.your-project.yaml` into runtime sibling YAMLs under
  `contexts/config/`
- Copies `project-context.your-project.md` to `contexts/project-context.md`
...
packs/your-project/
├── README.md                        ← this file
├── pipeline.your-project.yaml            ← THE TRIGGER MAP (heart of the pack)
├── project-context.your-project.md       ← prose context, loaded every run
├── rules/
│   ├── your-project-project-scope.mdc        ← Tier 1, alwaysApply
│   └── your-project-naming-conventions.mdc   ← Tier 1, alwaysApply
└── skills/
    ├── your-project-angular18-standards.md   ← Tier 2, on Layer trigger
    ├── your-project-angular19-standards.md   ← Tier 2, on Layer trigger
    ├── your-project-react-standards.md       ← Tier 2, on Layer trigger
    ├── your-project-vue3-standards.md        ← Tier 2, on Layer trigger
    ├── your-project-javascript-standards.md  ← Tier 2, on Layer trigger
    ├── your-project-typescript-standards.md  ← Tier 2, on Layer trigger
    └── your-project-java-standards.md        ← Tier 2, on Layer trigger
```
