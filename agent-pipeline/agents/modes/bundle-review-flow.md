---
name: bundle-review-flow
description: Review's bundle-mode flow. Loaded ONLY when review.md detect_bundle_mode (0c) finds frontmatter.mode == "bundle" in the resolved $CONTEXTS_FILE. Single-story / bug flow never reads this file.
---

# Review — Bundle Mode Flow

**Load ONLY when `detect_bundle_mode (0c)` in `review.md` sets `{bundle_mode} = true`.** Single-story and bug flows never read this file.
