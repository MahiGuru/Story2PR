...
2b. **Extract `{TICKET_ID}` from the trigger text** (runs BEFORE any ticket fetch).
   ...
   (a) Direct canonical token — ticket ID appearing as a standalone word:
       ...
       Matches: "Work on IIQMAG-1234"
                "Work on story IIQMAG-1234"        (extra descriptor tolerated)
                "Work on bug IIQMAG-1234 please"
                "@orchestrator.md Work on IIQMAG-1234"

   (b) URL path pattern — JIRA / Atlassian-style URLs:
       regex:  /browse/([A-Z][A-Z0-9]+-\d+)(?:[/?#]|$)
       Matches: "Work on https://acme.atlassian.net/browse/IIQMAG-1234"
                "Work on https://acme.atlassian.net/browse/IIQMAG-1234?..."
                "Work on https://jira.example.com/browse/IIQMAG-1234#comment-5"
   ...
   (c) Deep-link query-param pattern — JIRA board/sprint URLs:
       ...
       Matches: "https://acme.atlassian.net/jira/software/projects/PROJ/boards/1?selectedIssue=IIQMAG-1234"
...
4b. **Load AC template skill (v16 — eager loading for enrich_ac (A.5) consistency):**
   ...
   IF {ac_templates_skill} is set:
     Read: .cursor/skills/{ac_templates_skill}    # pack-specific (iiq / …)
   ELSE:
