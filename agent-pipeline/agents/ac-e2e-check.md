...
terminal_run  → Use existing spec files, no AI involved
                Cost: $0.00 per run (pure shell + framework)
                Requires: specs already generated (run e2e_generate first)
                Trigger: ./agent-pipeline/bin/run-e2e.sh {EPIC_ID}
                Use when: nightly regression, CI, any re-run
...
Nightly / CI  → ./agent-pipeline/bin/run-e2e.sh PROJ-EPIC-100
                 → runs ALL existing specs headless
                 → updates plan's Last Result columns
                 → cost: $0.00

After 30 nightly re-runs: still $0.00 (AI never re-engaged)
Only re-engage AI for: (a) new story's specs, (b) diagnosing failures
```
...
│ Verify:    {ai_browser | e2e_generate | terminal_run | both}   │
...
═══════════════════════════════════════════════════════════════
  ...
  💡 For zero-cost re-runs, use the terminal script:
     ./agent-pipeline/bin/run-e2e.sh {EPIC_ID}
     (Specs exist — no need to re-invoke AI for identical re-runs.)
...
## Phase: terminal_run (Phase T — when verify_mode = terminal_run)

**This is the default path for both story mode and epic mode when specs exist.** The agent opens a terminal via bash_tool, runs the test framework, and parses results. Minimal LLM tokens.

### Step: check_specs_exist (T-1)

```bash
FRAMEWORK=$(yaml_get demo.e2e.framework)
SPEC_DIR=$(yaml_get demo.e2e.${FRAMEWORK}.spec_dir)

# For story mode: check this story's specs
# For epic mode: check all stories' specs

STORY_MODE:
  find "$SPEC_DIR" -name "${TICKET_ID}-*.spec.*" -o -name "${TICKET_ID}-*.cy.*"
  IF empty → SPECS_EXIST=false → fall through to e2e_generate for this story

EPIC_MODE:
  For each story in plan's "Stories in plan" list:
    find "$SPEC_DIR" -name "${story_id}-*.spec.*" ...
    Collect missing list

  IF all specs exist → SPECS_EXIST=true
  IF some missing → prompt user:
    ⚠ Specs missing for: PROJ-1236, PROJ-1238
    Options:
      `Generate missing specs` — generate + run (costs ~$0.30)
      `Run what exists` — skip missing stories
      `Cancel`
```

### Step: terminal_pre_verify (T-2)

Build + health check via `pre_verify.shell_init + build_command`. No change from existing Step 1c.

### Step: execute_terminal_runner (T-3)

```bash
# Agent calls bash_tool directly — user doesn't have to run anything
bash_tool:
  command: "./agent-pipeline/bin/run-e2e.sh ${EPIC_ID}"
  description: "Run all E2E specs for ${EPIC_ID} headless"
```

**The script handles:**
- Running the framework command (Protractor/Cypress/Playwright) headless
- JUnit XML output parsing
- Updating Last Result columns in `epic-e2e-plan.md`
- Capturing screenshots (framework does this, configured in framework config)
- Prepending row to Run History

**Agent reads script output** — just a short text summary:

```
═══════════════════════════════════════════════════════════════
  E2E run complete for PROJ-EPIC-100
═══════════════════════════════════════════════════════════════
  Framework:    cypress
  Results:      15 / 18 passed, 2 failed, 1 skipped
  Plan updated: contexts/PROJ-EPIC-100/epic-e2e-plan.md
  Results dir:  contexts/PROJ-EPIC-100/test-results/2026-04-17_15-30/
  LLM tokens:   0 (no AI used)

  ⚠ 2 test(s) failed:
    - AC5: notification endpoint returned 404
    - Cross-story 2: certId null in notification
  Screenshots: contexts/screenshots/PROJ-EPIC-100/2026-04-17_15-30/
```

That's maybe 500 tokens — agent parses it into the gate message.

### Step: present_results_gate (T-4)

```
═══════════════════════════════════════════════════════════════
  TERMINAL RUN COMPLETE — {EPIC_ID}
═══════════════════════════════════════════════════════════════

  Results:   {P} / {T} passed, {F} failed, {S} skipped
  Duration:  {Xs}
  LLM cost:  $0.00 (terminal-only run)

  {IF failures:}
  Failed steps:
    • {test_name_1}   screenshot: epic_s1_05.png
    • {test_name_2}   screenshot: epic_s3_02.png

  Plan file:      contexts/{EPIC_ID}/epic-e2e-plan.md
  Screenshots:    contexts/screenshots/{EPIC_ID}/{run_date}/

> **👉 Pick one:**
> - `Show step {N} screenshot` — load ONE screenshot for visual check (~$0.05)
> - `Diagnose {test_name}` — AI analyzes failure root cause (~$0.20)
> - `Fix in surgeon` — route failure(s) to Surgeon for fixing
> - `Re-run failed only` — retry just failing specs in terminal (~$0.00)
> - `Show plan` — open epic-e2e-plan.md
> - `Done`
```

### Step: load_screenshots_on_demand (T-5)

Key point: screenshots live on disk. Agent loads them **only** when user asks:

```
User: "Show step 5 screenshot"
Agent:
  → read_image: contexts/screenshots/PROJ-EPIC-100/2026-04-17_15-30/epic_s1_05.png
  → displays it inline
  → ~$0.05 cost for this one image
```

Without that explicit ask, screenshots stay on disk. Plan file references them by path — user opens the plan in Cursor → clicks path → sees image in IDE preview. Zero LLM involved in that viewing path.
